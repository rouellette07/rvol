...
const getOI = async (ticker) => {
  const json = await getJson(
    'https://fapi.binance.com/futures/data/openInterestHist?period=5m&limit=500&symbol=' +
      ticker +
      'USDT'
  );
...
const getKlinesFut = async (ticker) => {
  const json = await getJson(
    'https://fapi.binance.com/fapi/v1/klines?interval=5m&limit=500&symbol=' +
      ticker +
      'USDT'
  );
  return json.map((x) => ({
...
const getKlinesSpot = async (ticker) => {
  const json = await getJson(
    'https://api.binance.com/api/v3/klines?interval=5m&limit=500&symbol=' +
      ticker +
      'USDT'
  );
  return json.map((x) => ({
